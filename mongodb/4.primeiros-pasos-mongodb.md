...
13. (BD: sample_mflix) Selecciona películas cun reparto de polo menos 5 actores.
```javascript
use sample_mflix
db.movies.find({
  cast: { $size: 5 }
...
14. (BD: sample_mflix) Selecciona películas onde algún actor teña idade >= 50.
```javascript
use sample_mflix
db.movies.find({
  actors: {
